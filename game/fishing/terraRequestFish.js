/*
	...
	cn_name: 中文名
	type: 种类标签
	obtained: 变量，记录是否持有这种鱼（已弃用）
	catching_place: 抓捕地点列表
	catching_place_cn: 抓捕地点列表的图标与中文名，用于图鉴
	icon: 图标
	angler_des: 渔夫对此的描述
	cute_angler_des: 可爱渔夫对此的描述
	catching_place_des: 捕获地点的显示内容
*/
setup.terraRequestFish = {
	"Amanita_Fungifin": {
		...
		catching_place_cn: "<<wolfcaveicon 'stream'>>狼窝的溪流<<print $options.artificial_fishing_pond ? '<br><<foresticon>>狼穴洞口附近的人工钓鱼池' : ''>>",
	...
	"Angelfish": {
		...
		catching_place_cn: "<<lakeicon 'pond'>>伊甸小屋的泉水<<print $options.artificial_fishing_pond ? '<br><<foresticon>>猎人小屋附近的人工钓鱼池' : ''>>",
	...
	"Batfish": {
		...
		catching_place_cn: "<<barbicon>>倒钩街的运河<br><<wolfcaveicon \"stream\">>狼窝的溪流<<print $options.artificial_fishing_pond ? '<br><<foresticon>>狼穴洞口附近的人工钓鱼池' : ''>>",
	...
	"Bunnyfish": {
		...
		catching_place_cn: "<<lakeicon 'fishing'>>森林湖钓鱼岩",
	...
	"Dirtfish": {
		...
		catching_place_cn: "<<lakeicon 'fishing'>>森林湖钓鱼岩<br><<barbicon>>倒钩街的运河",
	...
	"Fallen_Starfish": {
		...
		catching_place_cn: "<<lakeicon 'fishing'>>森林湖钓鱼岩<br><<lakeicon 'pond'>>伊甸小屋的泉水<<print $options.artificial_fishing_pond ? '<br><<foresticon>>猎人小屋附近的人工钓鱼池' : ''>>",
	...
	"Jewelfish": {
		...
		catching_place_cn: "<<barbicon>>倒钩街的运河<br><<birdicon 'rainwater'>>鹰塔的雨水池<<print $options.artificial_fishing_pond ? '<br><<birdicon \"tower\">>高塔塔底的人工钓鱼池' : ''>>",
	...
	"Unicorn_Fish": {
		...
		angler_des: "\"独角兽和彩虹都是好东西！它们无处不在，甚至在水中。真的，我居然在圣湖中看到一条独角兽鱼！你的任务就是把它钓起来，我会拿它当宠物！\"",
		cute_angler_des: "\"神圣之地真的是一个很美丽的地方，到处都是独角兽和彩虹这些奇幻美好的东西！我说真的！我甚至在湖里看到一条独角兽鱼！你能帮我抓一条回来吗？我想养一条独角兽当宠物！拜托拜托～\"",
		catching_place_des: "<<fishing_request_place_temple_garden>>"
    },
	"Wyverntail": {
        name: "Wyverntail",
	...
	"Bloody_Manowar": {
		...
		angler_des: "\"噢！别靠近我！我被一条血水母蜇了！怕你孤陋寡闻不知道血水母是什么，所以我告诉你，它是整个 <<if $saveName isnot \"\">>$saveName<<else>>$saveId<</if>> 中最凶险的水母！如果你敢，去那个森林湖，把它抓来！\"",
